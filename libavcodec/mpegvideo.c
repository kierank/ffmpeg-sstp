...
void ff_init_block_index(MpegEncContext *s){ //FIXME maybe rename
    ...
    s->dest[0] = s->current_picture.f->data[0] + (int)((s->mb_x - 1U) <<  mb_width);
    s->dest[1] = s->current_picture.f->data[1] + (int)((s->mb_x - 1U) << (mb_width - s->chroma_x_shift));
    s->dest[2] = s->current_picture.f->data[2] + (int)((s->mb_x - 1U) << (mb_width - s->chroma_x_shift));
